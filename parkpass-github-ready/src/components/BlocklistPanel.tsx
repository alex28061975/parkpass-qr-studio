...
export function BlocklistPanel({ isOpen, onClose, database = [] }: BlocklistPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden text-slate-200">
        ...
        <div className="p-4 bg-slate-950/70 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-rose-950/60 border border-rose-800/80 flex items-center justify-center text-rose-400 shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-rose-950 text-rose-300 border border-rose-800/60 font-mono font-bold px-2 py-0.5 rounded-full text-xs">
        ...
        {/* Toast feedback */}
        {toastMsg && (
          <div className="bg-slate-800 border-b border-slate-700 px-4 py-2 text-xs font-bold text-slate-200 flex items-center gap-2 animate-in fade-in">
            <span>{toastMsg}</span>
          </div>
        )}
        ...
        {showAddForm && (
          <form 
            ...
            className="p-4 bg-slate-950/90 border-b border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-3 text-xs animate-in slide-in-from-top-2 duration-200"
          >
            <div className="space-y-1">
              ...
              <input
                ...
                required
                className="w-full px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-white font-mono font-bold uppercase focus:outline-none focus:border-rose-500"
              />
            </div>

            <div className="space-y-1">
              <label className="font-bold text-slate-300">Driver Name</label>
              <input
                type="text"
                value={formDriverName}
                onChange={(e) => setFormDriverName(e.target.value)}
                placeholder="e.g. Sarah Connor"
                className="w-full px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-white focus:outline-none focus:border-rose-500"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-400">Hospital Site</label>
              <input
                type="text"
                value={formHospital}
                onChange={(e) => setFormHospital(e.target.value)}
                placeholder="e.g. Whipps Cross Hospital"
                className="w-full px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:border-rose-500"
              />
            </div>

            <div className="space-y-1">
              <label className="font-bold text-slate-400">Ward / Department</label>
              <input
                type="text"
                value={formWard}
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-400">Date Required</label>
              <input
                type="date"
                value={formDateRequired}
                onChange={(e) => setFormDateRequired(e.target.value)}
                className="w-full px-2.5 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:border-rose-500 font-mono"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-400">Form ID</label>
              <input
                type="text"
                value={formFormId}
        ...
        {/* Search Bar */}
        <div className="p-3 bg-slate-900/60 border-b border-slate-800 flex items-center justify-between gap-3">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-950 border border-slate-800 rounded-lg text-slate-200 focus:outline-none focus:border-rose-500"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        {/* Exact 12-Column Dark Table Layout:
            [FORM ID] [HOSPITAL SITE] [WARD / DEPT] [DATE REQUIRED] [EXPIRY DATE] [VRM] [DRIVER NAME] [PHONE] [EMAIL] [VOUCHER CODE] [REASON] [ACTION]
        */}
        <div className="overflow-x-auto flex-1 max-h-[60vh] bg-slate-950">
          <table className="min-w-[1200px] w-full text-left text-xs border-collapse table-auto">
            <thead className="bg-slate-900/95 text-slate-300 font-bold sticky top-0 z-10 uppercase text-[10px] tracking-wider border-b border-slate-800">
              <tr>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">FORM ID</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">HOSPITAL SITE</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">WARD / DEPT</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">DATE REQUIRED</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">EXPIRY DATE</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">VRM</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">DRIVER NAME</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">PHONE</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">EMAIL</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">VOUCHER CODE</th>
                <th className="p-3 whitespace-nowrap border-r border-slate-800/80">REASON</th>
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item, idx) => (
                  <tr
                    key={item.id || idx}
                    className="hover:bg-rose-950/20 transition-colors"
                  >
                    {/* 1. FORM ID */}
                    <td className="p-3 font-bold text-rose-400 whitespace-nowrap border-r border-slate-800/60">
                      <span className="bg-rose-950/60 px-1.5 py-0.5 rounded border border-rose-800/60">
                        {item.formId ? `#${formatFormId(item.formId)}` : "-"}
                      </span>
                    </td>

                    {/* 2. HOSPITAL SITE */}
                    <td className="p-3 font-sans text-slate-300 whitespace-nowrap border-r border-slate-800/60">
                      {item.hospitalSite || "-"}
                    </td>
                    ...
                    {/* 4. DATE REQUIRED */}
                    <td className="p-3 text-slate-300 whitespace-nowrap border-r border-slate-800/60 font-mono">
                      {item.dateRequired || "-"}
                    </td>
                    ...
                    {/* 6. VRM */}
                    <td className="p-3 font-bold text-white uppercase tracking-wider whitespace-nowrap border-r border-slate-800/60">
                      <span className="bg-rose-950/80 text-rose-300 px-2 py-0.5 rounded border border-rose-800/80 font-mono font-extrabold text-[12px]">
                        {item.vrm}
                      </span>
                    </td>

                    {/* 7. DRIVER NAME */}
                    <td className="p-3 font-sans font-semibold text-slate-200 whitespace-nowrap border-r border-slate-800/60">
                      {item.driverName ? toTitleCase(item.driverName) : "-"}
                    </td>

                    {/* 8. PHONE */}
                    <td className="p-3 text-slate-400 whitespace-nowrap border-r border-slate-800/60">
                      {item.phone || "-"}
                    </td>
                    ...
                    {/* 10. VOUCHER CODE */}
                    <td className="p-3 whitespace-nowrap border-r border-slate-800/60">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10.5px] font-bold border border-rose-900/80 bg-rose-950/60 text-rose-300">
                        {item.voucherCode || "BLOCKED"}
                      </span>
                    </td>

                    {/* 11. REASON */}
                    <td className="p-3 font-sans text-slate-300 max-w-xs truncate border-r border-slate-800/60" title={item.reason}>
                    ...
                    <td className="p-3 text-center whitespace-nowrap">
                      <button
                        ...
                        className="px-2.5 py-1 bg-red-950/60 hover:bg-red-900/80 text-red-300 border border-red-800/60 text-[10px] font-bold uppercase tracking-wider rounded transition-all flex items-center gap-1 mx-auto cursor-pointer shadow-xs active:scale-95"
        ...
        <div className="p-3 bg-slate-950 border-t border-slate-800 text-xs text-slate-400 flex items-center justify-between font-sans">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-lg transition-colors cursor-pointer text-xs"
          >
            Done
          </button>
        </div>
